...
class TestMcpMainController(common.HttpCase):
    ...
    def setUp(self):
        ...
        self.env["ir.config_parameter"].sudo().set_param("mcp_server.use_api_keys", "True")
    ...
    def test_api_keys_disabled_mode(self):
        """Test endpoints work when API keys are disabled"""
        self.env["ir.config_parameter"].sudo().set_param("mcp_server.use_api_keys", "False")

        # Test system info works without API key
        response = self.url_open("/mcp/system/info")
        self.assertEqual(response.status_code, 200)

        # Test auth validation works without API key
        response = self.url_open("/mcp/auth/validate")
        self.assertEqual(response.status_code, 200)

        data = json.loads(response.content.decode())
        self.assertTrue(data["data"]["valid"])
        # Should return public user ID
        public_user = self.env.ref("base.public_user")
        self.assertEqual(data["data"]["user_id"], public_user.id)
